...
//                    if (candidate_id == 0) continue;
    ...
    template <bool has_deletions, bool collect_metrics = false>
    std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> searchBaseLayerMulti(std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst>* startingKnns, const void *data_point, size_t ef, BaseFilterFunctor* isIdAllowed = nullptr) {
	VisitedList *vl = visited_list_pool_->getFreeVisitedList();
	vl_type *visited_array = vl->mass;
	vl_type visited_array_tag = vl->curV;
	...
	dist_t lowerBound;
	while (!startingKnns.empty()) {
		std::pair<dist_t, tableint> initPair = startingKnns.top();
		tableint ep_id = initPair.second;
		if ((!has_deletions || !isMarkedDeleted(ep_id)) && ((!isIdAllowed) || (*isIdAllowed)(getExternalLabel(ep_id)))) {
			// trying to get a valid point -> do for each point in knnset
			dist_t dist = fstdistfunc_(data_point, getDataByInternalId(ep_id), dist_func_param_);
			top_candidates.emplace(dist, ep_id);
			lowerBound = dist;
			candidateSet.emplace(-dist, ep_id);
	...
	while(!candidateSet.empty()) {
		std::pair<dist_t, tableint> curr_el_pair = candidateSet.top();
		if ((-curr_el_pair.first) > lowerBound && (top_candidates.size() == ef || (!isIdAllowed && !has_deletions))) {
			break;
		}
		...
		tableint curNodeNum = curr_el_pair.second;
		std::unique_lock <std::mutex> lock(link_list_locks_[curNodeNum]);
		int *data = (int*) get_linklist0(curr_el_pair);
		size_t size = getListCount((linklistsizeint*) data);
    ...
    */
     ...
     * }
    */
    std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> deepSearch(tableint q) {
	    std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>>* knns = getKNNSet(q);
	    int ef_construction_ = 100;
	    std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> newKnns = searchBaseLayerMulti(knns, getDataByInternalId(q), 100);
	    delete knns;
	    return newKnns;			
    }
