...
//                    if (candidate_id == 0) continue;
    ...
    */
     ...
     * }
    */
    std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>> deepSearch(tableint q) {
	    std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>>* knns = getKNNSet(q);
	    int ef_construction_ = 100;
	    std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>> newKnns = searchBaseLayerMulti(knns, getDataByInternalId(q), 100);
	    delete knns;
	    return newKnns;			
    }
